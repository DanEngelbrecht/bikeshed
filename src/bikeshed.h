...
struct Bikeshed_Task_private
{
    ...
    Bikeshed_DependencyIndex_private    m_FirstDependencyIndex;
...
#define BIKESHED_SIZE(max_task_count, max_dependency_count) \
...
int Bikeshed_ExecuteOne(Bikeshed shed);
...
inline void Bikeshed_PoolPush_private(struct Bikeshed_Pool_private* pool, uint32_t index)
{
    ...
    pool->m_Head[index]     = (long)(BIKESHED_TASK_INDEX_PRIVATE(current_head));
...
int Bikeshed_ExecuteOne(Bikeshed shed)
{
    uint32_t task_index = Bikeshed_PoolPop_private(&shed->m_ReadyQueue);
    if (task_index == 0)
    {
        return 0;
    }
